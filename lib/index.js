...
function completePreviousPipeline (id, startedAt, completedTime) {
	var request =  http.request({
		...
		_.each(response.hits.hits, function(item) {
			completePreviousPipeline(item._id, item._source['@timestamp']);
		});
	});
	...
	request.write(JSON.stringify({
		...
		releaseStatus.timeInMs: completedTime.diff(startedAt, 'ms');
	}));
...
function completePreviousPipelineRuns(group, pipeline, startedAt) {
	...
	var request =  http.request({
		...
		_.each(response.hits.hits, function(item) {
			...
			completePreviousPipeline(item._id, moment(item._source.startedAt), item._source['@timestamp']);
		});
	});
